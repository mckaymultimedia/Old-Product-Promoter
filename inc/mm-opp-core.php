<?php
function mm_opp_old_product_promoter () {
...
function mm_opp_promote_old_post () {
    ...
    if ($omitCats!='') {
    	$sql = $sql."AND NOT(ID IN (SELECT tr.object_id 
                                    FROM $wpdb->terms  t 
                                          inner join $wpdb->term_taxonomy tax on t.term_id=tax.term_id and tax.taxonomy='product_cat' 
    ...
    $sql = $sql."            
            ORDER BY RAND() 
            LIMIT 1 ";
	$oldest_product = $wpdb->get_var($sql);   
	if (isset($oldest_product)) {
		mm_opp_update_old_post($oldest_product);
	}
}

function mm_opp_update_old_post($oldest_product) {
	global $wpdb;
	$post = get_post($oldest_product);
	$origPubDate = get_post_meta($oldest_product, 'mm_opp_original_pub_date', true); 
	if (!(isset($origPubDate) && $origPubDate!='')) {
	    $sql = "SELECT post_date from ".$wpdb->posts." WHERE ID = '$oldest_product'";
		$origPubDate=$wpdb->get_var($sql);
		add_post_meta($oldest_product, 'mm_opp_original_pub_date', $origPubDate);
		$origPubDate = get_post_meta($oldest_product, 'mm_opp_original_pub_date', true); 
	}
	...
	$sql = "UPDATE $wpdb->posts SET post_date = '$new_time',post_date_gmt = '$gmt_time',post_modified = '$new_time',post_modified_gmt = '$gmt_time' WHERE ID = '$oldest_product'";		
	$wpdb->query($sql);
	if (function_exists('wp_cache_flush')) {
		wp_cache_flush();
	}		
		
	$permalink = get_permalink($oldest_product);
